...
foreach ($sshConfig as $ssh) {
    if($keyword === null) {
        addItem('remove' . $ssh['ssh'], json_encode($ssh, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
            '删除ssh: ' . json_encode($ssh, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), $ssh['ssh']);
